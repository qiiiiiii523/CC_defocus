    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if 'lq' in train_data:
                ...
                # z-stack triplet dataset: keys are lq1/lq2/lq3/gt; pass through
                gt = train_data['gt']
                if mini_batch_size < batch_size:
                    indices = random.sample(range(0, batch_size), k=mini_batch_size)
                    train_data = {k: (v[indices] if torch.is_tensor(v) else v)
                ...
                if mini_gt_size < gt_size:
                    ...
                    for k, v in train_data.items():
                        if k == 'gt':
                            sliced[k] = v[:, :, x0*scale:x1*scale, y0*scale:y1*scale]
                        elif k.startswith('lq') and torch.is_tensor(v):
                            sliced[k] = v[:, :, x0:x1, y0:y1]
                        else:
                            sliced[k] = v
                    train_data = sliced
