...
def img2tensor_tif(imgs, float32=True):
    """Numpy array to tensor.
    ...
    if isinstance(imgs, list):
        return [_totensor_tif(img, float32) for img in imgs]
    else:
        return _totensor_tif(imgs, float32)
...
def padding_DP4(img_lqL, img_lqR, img_gt, img_gt1, gt_size):
    h, w, _ = img_gt.shape
    ...
    if h_pad == 0 and w_pad == 0:
        return img_lqL, img_lqR, img_gt, img_gt1

    img_lqL = cv2.copyMakeBorder(img_lqL, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_lqR = cv2.copyMakeBorder(img_lqR, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt  = cv2.copyMakeBorder(img_gt,  0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt1  = cv2.copyMakeBorder(img_gt1,  0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    # print('img_lq', img_lq.shape, img_gt.shape)
    return img_lqL, img_lqR, img_gt, img_gt1
